...
For browser-based applications that need to perform direct uploads to R2 storage, you can generate temporary upload tokens using your API key.

### When to Use Upload Tokens

<AccordionGroup>
<Accordion title="✅ Direct Browser Uploads">
  When implementing file uploads directly from the browser to R2 storage without exposing your main API key.
</Accordion>

<Accordion title="✅ Secure Client-Side Operations">
...
<Step title="Use Upload Token">
  Use the upload token for R2 upload operations.
...
<Accordion title="🛡️ Upload Token Security">
...
async function testUploadTokenGeneration(apiKey) {
  try {
    ...
    if (response.ok) {
      ...
      console.log('❌ Upload token generation failed:', response.status);
      return null;
    }
  } catch (error) {
    console.log('❌ Upload token generation error:', error.message);
    return null;
  }
}
```
