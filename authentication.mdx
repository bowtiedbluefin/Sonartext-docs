...
description: 'Learn how to authenticate with the Sonartext API using API keys and Firebase tokens'
---

The Sonartext API supports two authentication methods depending on your use case: API keys for server-to-server communication and Firebase tokens for browser-based applications.

## API Key Authentication (Recommended)

API keys are the primary authentication method for server-to-server integrations and most use cases.
...
<Step title="Generate a Key">
  Click "Generate New Key" to create your API key. Keys start with `stx_live_` for production or `stx_test_` for testing.
</Step>
...
const response = await fetch('/v1/audio/transcriptions', {
  ...
  body: JSON.stringify({
    ...
    r2_key: 'uploads/user123/audio.wav',
    model: 'whisper-1'
  })
});
```
...
## Firebase Authentication (Browser Apps)

For browser-based applications that need to perform direct uploads to R2 storage, you can use Firebase authentication.

### When to Use Firebase Auth

<AccordionGroup>
<Accordion title="✅ Direct Browser Uploads">
  When implementing file uploads directly from the browser to R2 storage without going through your server.
</Accordion>

<Accordion title="✅ User-Specific Access">
  When you need to associate uploads with specific authenticated users in your Firebase project.
</Accordion>

<Accordion title="✅ R2 Multipart Uploads">
  For the `/v1/uploads/*` endpoints that support multipart uploads to R2.
</Accordion>
</AccordionGroup>
...
<Step title="Configure Firebase">
  Set up Firebase Authentication in your project and configure your Firebase project with Sonartext.
</Step>

<Step title="Authenticate Users">
  Use Firebase Auth to sign in users and get ID tokens.
...
<Step title="Use ID Token">
  Use the Firebase ID token for R2 upload operations.
...
| **Firebase Token** | Browser uploads, user authentication | Upload endpoints primarily | Client & server side |
...
<Accordion title="🛡️ Firebase Token Security">
- **Validate tokens server-side** before performing sensitive operations
- **Use token refresh** to maintain valid authentication
...
SONARTEXT_TEST_API_KEY=stx_test_1234567890abcdef...
...
<Tab title="Test Upload Token">
```javascript
// Test Firebase token for upload operations
async function testUploadToken(firebaseIdToken) {
  try {
    ...
    if (response.ok) {
      ...
      console.log('✅ Upload token generated:', data.expiresAt);
      return data;
    } else {
      console.log('❌ Upload token test failed:', response.status);
      return null;
    }
  } catch (error) {
    console.log('❌ Upload token test error:', error.message);
    return null;
  }
}
```
...
function authenticateAPI(req, res, next) {
  ...
  if (!apiKey.startsWith('stx_live_') && !apiKey.startsWith('stx_test_')) {
...
def authenticate_request():
    ...
    if not (api_key.startswith('stx_live_') or api_key.startswith('stx_test_')):
...
```javascript
// Complete browser upload flow with Firebase auth
class SonartextUploader {
  ...
  async uploadFile(file, transcriptionOptions = {}) {
    ...
    // Complete upload
    const completeResponse = await fetch('/v1/uploads/complete', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${this.uploadToken}`,
        'Content-Type': 'application/json'
