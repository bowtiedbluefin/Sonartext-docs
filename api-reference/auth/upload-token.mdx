...
Generate a short-lived token for browser uploads. This endpoint uses API key authentication to create temporary tokens for client-side upload operations.

## Authentication

Use your Sonartext API key to generate upload tokens:
...
2. **Upload file parts** directly to cloud storage using the presigned URLs
...
<Note>
This endpoint is primarily designed for browser-based applications that need to perform uploads directly to cloud storage. Server-to-server integrations can often use API keys directly with other endpoints.
</Note>
