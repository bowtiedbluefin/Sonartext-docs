...
Start a multipart upload for large files (>100MB recommended). Returns presigned URLs for uploading file parts directly to Cloudflare R2.

## Benefits of R2 Multipart Uploads
...
<Card title="Zero Bandwidth Costs" icon="coins">
  Direct uploads to R2 bypass API bandwidth limits
</Card>
...
<Card title="Browser Direct Upload" icon="browser">
  Upload directly from browser to R2, bypassing API limits
</Card>
</CardGroup>
...
<ResponseField name="r2UploadId" type="string" required>
  R2 internal upload ID used by Cloudflare R2.
  
  Example: `"2~Vg5SJ5RRQON9vnefrO2XgB"`
</ResponseField>

<ResponseField name="r2Key" type="string" required>
  R2 object key where your file will be stored.
