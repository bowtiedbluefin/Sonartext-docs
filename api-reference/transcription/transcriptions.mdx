...
```python Storage Re-transcription
import requests

def retranscribe_from_storage(storage_key, api_key, **options):
...
result = retranscribe_from_storage(
    storage_key='uploads/user123/1234567890-audio.wav',
...
<Step title="Initial Upload">
  Upload a large file using multipart upload and get the `storageKey`.
  
  ```javascript
  const uploadResult = await completeUpload(uploadId, parts);
  const storageKey = uploadResult.storageKey; // Save this!
  ```
</Step>
...
<Step title="Re-transcribe with Different Settings">
  Use the saved `storageKey` to transcribe the same file with different parameters.
...
<Accordion title="🎯 Choose the Right Method">
  ...
  - **Storage reference**: Re-transcribing uploaded files with different settings
...
<Accordion title="⚡ Performance Tips">
  ...
  - **Caching**: Cache results and use storage re-transcription for different formats
</Accordion>

<Accordion title="💰 Cost Optimization">
  - **Re-use uploads**: Save `storageKey` from multipart uploads for multiple transcriptions
...
<ResponseExample>
```json Storage Object Not Found
{
  "error": "storage_object_not_found",
  "message": "Specified storage object could not be found",
  "details": {
